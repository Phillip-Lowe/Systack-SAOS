...
# vhp-8c-16gb-amd = High Performance, 8 vCPU, 16GB RAM ($96/mo)
TIER_PLANS = {
    "business": {
    ...
    "enterprise": {
        ...
        "region": "ord",
        "os_id": UBUNTU_22_04_OS_ID,
        "description": "Enterprise Fleet - 8 vCPU, 16GB RAM + Dedicated Support"
    },
...
def generate_cloud_init(client_id: str, tier: str, agent_name: str, tailscale_auth_key: str) -> str:
    ...
    # Build with string concatenation to avoid triple-quote issues
    cloud_init = "#cloud-config\n"
    cloud_init += "package_update: true\n"
    cloud_init += "package_upgrade: true\n"
    cloud_init += "\n"
    cloud_init += "packages:\n"
    cloud_init += "  - curl\n  - wget\n  - git\n  - htop\n  - tmux\n"
    cloud_init += "  - ufw\n  - fail2ban\n  - nginx\n  - docker.io\n"
    cloud_init += "  - docker-compose\n  - python3\n  - python3-pip\n"
    cloud_init += "  - python3-venv\n  - jq\n"
    cloud_init += "\n"
    cloud_init += "users:\n"
    cloud_init += "  - name: systack\n"
    cloud_init += "    groups: [docker, sudo]\n"
    cloud_init += "    shell: /bin/bash\n"
    cloud_init += "    sudo: ['ALL=(ALL) NOPASSWD:ALL']\n"
    cloud_init += "    ssh_authorized_keys: []\n"
    cloud_init += "\n"
    cloud_init += "runcmd:\n"
    cloud_init += '  - echo "=== SAOS Provisioning Started ===" > /var/log/saas-provision.log\n'
    cloud_init += '  - date >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += '  # Install Tailscale (join without tags - configure ACLs in Tailscale admin)\n'
    cloud_init += '  - curl -fsSL https://tailscale.com/install.sh | sh\n'
    cloud_init += f'  - tailscale up --authkey {tailscale_auth_key} --hostname saos-{client_id} --advertise-tags tag:saos-client >> /var/log/saas-provision.log 2>&1 || echo "Tailscale join failed - check key ACLs" >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += '  # Install Ollama\n'
    cloud_init += '  - curl -fsSL https://ollama.com/install.sh | sh\n'
    cloud_init += '  - systemctl enable ollama\n'
    cloud_init += '  - systemctl start ollama\n'
    cloud_init += '\n'
    cloud_init += '  # Pull default model (tier-appropriate) - use ollama user\n'
    cloud_init += '  - (sleep 5; su - ollama -s /bin/bash -c "ollama pull qwen2.5:7b") >> /var/log/saas-provision.log 2>&1 || echo "Model pull failed" >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += '  # Install n8n via Docker\n'
    cloud_init += '  - docker pull n8nio/n8n:latest >> /var/log/saas-provision.log 2>&1 || echo "n8n docker pull failed" >> /var/log/saas-provision.log\n'
    cloud_init += '  - docker run -d --name n8n -p 5678:5678 -v /opt/n8n:/home/node/.n8n n8nio/n8n:latest >> /var/log/saas-provision.log 2>&1 || echo "n8n start failed" >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += '  # Install OpenClaw via npm (get.openclaw.ai is not a real domain)\n'
    cloud_init += '  - npm install -g @openclaw/cli 2>/dev/null || echo "OpenClaw npm install failed - install manually" >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += '  # Configure firewall\n'
    cloud_init += '  - ufw allow 22/tcp\n'
    cloud_init += '  - ufw allow 80/tcp\n'
    cloud_init += '  - ufw allow 443/tcp\n'
    cloud_init += '  - ufw allow 5678/tcp\n'
    cloud_init += '  - ufw allow 11434/tcp\n'
    cloud_init += '  - ufw allow 18789/tcp\n'
    cloud_init += '  - ufw --force enable\n'
    cloud_init += '\n'
    cloud_init += '  # Configure fail2ban\n'
    cloud_init += '  - systemctl enable fail2ban\n'
    cloud_init += '  - systemctl start fail2ban\n'
    cloud_init += '\n'
    cloud_init += '  - |\n'
    cloud_init += '    # Configure Tailscale Serve for public HTTPS access (if tailscale joined)\n'
    cloud_init += '    if tailscale status >/dev/null 2>&1; then\n'
    cloud_init += '      tailscale serve --https=443 off 2>/dev/null || true\n'
    cloud_init += '      tailscale serve --https=443 / http://localhost:5678 2>/dev/null || true\n'
    cloud_init += '      TAILSCALE_IP=$(tailscale ip -4 2>/dev/null || echo "")\n'
    cloud_init += '      TAILSCALE_DNS=$(tailscale status --json 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get(\x27Self\x27,{}).get(\x27DNSName\x27,\x27\x27))" || echo "")\n'
    cloud_init += '    else\n'
    cloud_init += '      TAILSCALE_IP=""\n'
    cloud_init += '      TAILSCALE_DNS=""\n'
    cloud_init += '    fi\n'
    cloud_init += '\n'
    cloud_init += '    # Signal completion via webhook\n'
    cloud_init += '    curl -X POST "https://n8n.systack.net/webhook/saos-provision" \\\n'
    cloud_init += '      -H "Content-Type: application/json" \\\n'
    cloud_init += f'      -d \x27{{"client_id":"{client_id}","vps_ip":"\x27$(curl -s ifconfig.me)\x27","tailscale_ip":"\x27"$TAILSCALE_IP"\x27","tailscale_dns":"\x27"$TAILSCALE_DNS"\x27","status":"ready","timestamp":"\x27$(date -Iseconds)\x27"}}\x27 \\\n'
    cloud_init += '      || echo "Webhook callback failed" >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += '  - echo "=== SAOS Provisioning Complete ===" >> /var/log/saas-provision.log\n'
    cloud_init += '  - date >> /var/log/saas-provision.log\n'
    cloud_init += '\n'
    cloud_init += "# Write status file for health checks\n"
    cloud_init += "write_files:\n"
    cloud_init += "  - path: /etc/saos-provision-status.json\n"
    cloud_init += '    content: |\n'
    cloud_init += f'      {{ "client_id": "{client_id}", "tier": "{tier}", "agent_name": "{agent_name}", "provisioned_at": "$(date -Iseconds)", "status": "in_progress" }}\n'
