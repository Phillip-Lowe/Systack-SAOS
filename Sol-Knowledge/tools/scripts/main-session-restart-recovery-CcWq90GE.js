...
import { t as createSubsystemLogger } from "./subsystem-LmgZvqF7.js";
import { i as callGateway } from "./call-NAbH1Jz8.js";
import { i as resolveSessionFilePath, s as resolveSessionTranscriptPathInDir } from "./paths-RbVDOqFp.js";
import { t as loadSessionStore } from "./store-load-DjaY7SAt.js";
import { s as updateSessionStore } from "./store-CmwzJ9N6.js";
import { t as resolveAgentSessionDirs } from "./session-dirs-CpmO9wCO.js";
import { i as resolveAllAgentSessionStoreTargetsSync } from "./targets-DGxp4nvt.js";
import "./sessions-BDnaolrd.js";
import { l as readSessionMessagesAsync } from "./session-utils.fs-D_3ZT9EK.js";
import { t as sanitizePendingFinalDeliveryText } from "./pending-final-delivery-XNH_y2B4.js";
import { m as resolveGatewaySessionStoreTarget } from "./session-utils-DiEYEgiu.js";
...
// SOL PATCH: Rebuild session store from orphaned .jsonl files on disk
async function rebuildStoreFromDisk(storePath) {
	const sessionsDir = path.dirname(storePath);
	let files;
	try {
		files = await fs.promises.readdir(sessionsDir);
	} catch (err) {
		if (err.code === "ENOENT") return { added: 0 };
		throw err;
	}
	...
	if (jsonlFiles.length === 0) return { added: 0 };
	let added = 0;
	await updateSessionStore(storePath, (store) => {
		for (const filename of jsonlFiles) {
			const sessionId = filename.replace(".jsonl", "");
			const sessionFile = path.join(sessionsDir, filename);
			// Check if already in store by sessionId
			let exists = false;
			for (const key of Object.keys(store)) {
				if (store[key]?.sessionId === sessionId) {
					exists = true;
					break;
				}
			}
			if (exists) continue;
			// Check file has meaningful content (> 100 bytes)
			try {
			...
			// Re-register with status running + abortedLastRun so recovery picks it up
			const sessionKey = `agent:sol:main:${sessionId}`;
			store[sessionKey] = {
				sessionId,
				sessionFile,
				status: "running",
	...
	if (added > 0) log.info(`rebuildStoreFromDisk: re-registered ${added} session(s) for recovery`);
	return { added };
}
// END SOL PATCH
...
async function recoverRestartAbortedMainSessions(params = {}) {
	...
	for (const storePath of await resolveRestartRecoveryStorePaths(params)) {
		// SOL PATCH: Rebuild store from disk before recovery
		try {
			const rebuildResult = await rebuildStoreFromDisk(storePath);
			if (rebuildResult.added > 0) log.info(`store rebuild added ${rebuildResult.added} session(s) for ${storePath}`);
		} catch (err) {
			log.warn(`store rebuild failed for ${storePath}: ${String(err)}`);
		}
		// END SOL PATCH
