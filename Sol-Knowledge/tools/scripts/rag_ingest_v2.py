#!/usr/bin/env python3
"""
RAG Ingestion Pipeline for SOL (v2 — avoids macOS flock deadlock)
Copies files to temp, reads from temp, avoids iCloud/Obsidian lock contention.
"""

import os, sys, argparse, hashlib, json, shutil, tempfile
...
VAULT_PATHS = [
    os.path.expanduser("~/.openclaw/workspaces/sol"),       # Sol workspace memory (accessible)
]

EXCLUDE_DIRS = {
    "node_modules", ".git", ".obsidian", "_attachments", "_views",
    "main-backup", "reports", "plugin-skills", "skills", "tmp", "temp",
}
...
# ── SAFE FILE READ (avoids flock deadlock) ──────────────────────────
def safe_read_text(filepath: Path) -> str:
    """Copy to temp then read — avoids macOS advisory lock issues."""
    with tempfile.NamedTemporaryFile(mode='wb', delete=True) as tmp:
        # Use shutil.copyfileobj which doesn't use flock
...
def should_exclude(path: Path, vault_root: Path) -> bool:
    """Exclude if any directory part AFTER the vault root matches an exclude dir."""
    try:
